const anzhiyu = {
  ...
  switchConsole: () => {
    ...
    if (consoleEl.classList.contains("show")) {
      // 控制台隐藏时
      rightSide.style.display = "flex"; // 显示右侧面板
      nav.style.backdropFilter = "blur(5px)"; // 恢复模糊效果
      nav.style.webkitBackdropFilter = "blur(5px)"; // 恢复模糊效果
    } else {
      // 控制台显示时
      rightSide.style.display = "none"; // 隐藏右侧面板
      nav.style.backdropFilter = "none"; // 去除模糊效果
    }
...
const MortalCat = {
  ...
  showWelcome: (ipStore) => {
    ...
    let dist = MortalCat.getDistance(121.413921, 31.08929, ipStore.result.location.lng, ipStore.result.location.lat),
      address,
      welcome_info;
    //根据国家、省份、城市信息自定义欢迎语
    //海外地区不支持省份及城市信息
