const anzhiyu = {
  ...
  hideConsole: function () {
    // 获取consoleEl的classList
    const consoleClassList = consoleEl.classList;

    // 检查并移除控制台相关的class
    if (consoleClassList.contains("show") || consoleClassList.contains("reward-show")) {
      // 移除所有匹配的类名
      consoleClassList.remove("show");
      consoleClassList.remove("reward-show");
      // 显示rightside并添加模糊效果
      document.getElementById('rightside').style.display = 'flex';
      document.getElementById('nav').style.backdropFilter = 'blur(5px)';
      document.getElementById('nav').style.webkitBackdropFilter = 'blur(5px)';
    }

    // 获取center-console元素并取消选中状态（如果被选中）
    const centerConsole = document.getElementById("center-console");
    if (centerConsole && centerConsole.checked) {
  ...
  addEventListenerMusic: function () {
    ...
    anMusicBtnGetSong.addEventListener("click", () => {
      ...
      // 黑胶唱片状态
      document.querySelector(".aplayer-button").classList.contains("aplayer-pause") ? document.querySelector(".Mo-song-disc").classList.add("Mo-play") : document.querySelector(".Mo-song-disc").classList.remove("Mo-play");
  ...
  switchConsole: () => {
    ...
    // 检查控制台的显示状态并切换
    const isConsoleShown = consoleEl.classList.contains("show");

    rightSide.style.display = isConsoleShown ? "flex" : "none";
    const blurValue = isConsoleShown ? "blur(5px)" : "none";
...
const MortalCat = {
    ...
    try {
      if (!ipLoacation) {
        return new Promise((resolve, reject) => {
          ...
          var url = `https://apis.map.qq.com/ws/location/v1/ip?key=4WGBZ-5LULH-GQKDK-WLEVH-QLMEV-DVFJS&output=jsonp`;
          script.src = url;
          ...
          window.QQmap = (data) => {
            if (data.status === 0) {
              ...
              MortalCat.showWelcome(ipLoacation);
              resolve();
            } else {
  ...
  showWelcome: (ipStore) => {
    ...
    let dist = MortalCat.getDistance(116.554033, 31.705385, ipStore.result.location.lng, ipStore.result.location.lat),
      address,
      welcome_info;
    //根据国家、省份、城市信息自定义欢迎语
    //海外地区不支持省份及城市信息
    ...
    🗺️您距离 <strong>Mo</strong> 约有 <strong>${dist}</strong> 公里！<br>
    当前IP地址为：<br>
